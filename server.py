...
# Языки и «чужие пласты», из которых черновик берёт материал. На каждый запрос
# случайные несколько: иначе модель всегда лезет в одну и ту же латынь.
NAME_LANGS = [
    "латынь", "древнегреческий", "итальянский", "немецкий", "японский",
    "исландский", "тюркские языки", "французский", "испанский",
    "старославянский", "финский", "нидерландский", "иврит", "санскрит",
]

NAME_STRATA = [
    "мифология", "астрономия и названия созвездий", "минералы и камни",
    "анатомия", "морское дело", "ботаника", "метеорология", "алхимия",
    "геральдика", "музыкальные термины", "кулинария", "типографика",
]
...
LAB_DRAFT_NAMES = (
    "\n\nСЕЙЧАС ТЫ ЧЕРНОВИК — человек его не увидит. Работай строго в два такта.\n\n"
    "ТАКТ 1 — МАТЕРИАЛ. Выпиши списком, без красот:\n"
    "— 3–5 ядер запроса: главные существительные и главное действие;\n"
    "— к каждому ядру 3–4 синонима по-русски, включая разговорные и устаревшие;\n"
    "— перевод каждого ядра на эти языки: {langs}. "
    "Записывай и оригиналом, и как звучит по-русски;\n"
    "— профжаргон области: как это называют те, кто работает руками;\n"
    "— чужой пласт ({strata}): слова оттуда с этим же смыслом или похожей формой.\n\n"
    "ТАКТ 2 — ОПЕРАЦИИ. Собери РОВНО 10 названий из материала такта 1. "
    "Каждую операцию применить хотя бы раз:\n"
    "1) СПЛАВ ПО ОБЩЕМУ СЛОГУ — два слова срастаются на общем куске "
    "(мониторинг + terra = МонТерра);\n"
    "2) СЛОВО ВНУТРИ СЛОВА — обычное слово, внутри которого целиком лежит ключевое "
    "(обод → Ободрение);\n"
    "3) СДВИГ ОДНОЙ БУКВЫ — слово меняет смысл от одной замены "
    "(специалист → Спицалист);\n"
    "4) ЧУЖОЕ СЛОВО ЦЕЛИКОМ — короткое иностранное, которое по-русски звучит как своё;\n"
    "5) ЖАРГОНИЗМ В ЛОБ — одно слово из профессии, понятное своим (Люфт);\n"
    "6) ПЕРЕРАЗБОР — читается двумя способами в зависимости от того, где резать.\n\n"
    "КАЧЕСТВО ШВА (для операций 1–3): сплав годится, только если читается с первого "
    "раза и обе исходные половины остаются узнаваемы. «база + азимут = Базимут» — "
    "годится. Комок, который приходится расшифровывать, — нет, выбрасывай сразу.\n\n"
    "ЗАПРЕТ: слова из примеров выше (МонТерра, Ободрение, Спицалист, Люфт) — "
    "это иллюстрации операций, а не заготовки. Использовать их в ответе нельзя.\n\n"
    "ЖЁСТКИЕ РАМКИ для каждого названия: одно слово, максимум два. "
    "Читается со слуха и записывается без переспроса. "
    "НЕ предложение, НЕ восклицание, НЕ цитата, НЕ строчка из песни.\n\n"
    "Выведи сначала материал, потом строку «—НАЗВАНИЯ—», потом десять названий, "
    "каждое с новой строки, без нумерации и без объяснений. "
    "Слабые пиши тоже: отбор будет потом."
)

LAB_PICK_NAMES = (
    "\n\nПеред тобой запрос и черновик. Выбирай ТОЛЬКО из списка после «—НАЗВАНИЯ—». "
    "Возьми ТРИ, разных по операции, а не три вариации одного.\n"
    "ГЛАВНЫЙ ТЕСТ: прочитай каждое название ОТДЕЛЬНО — без запроса, без контекста, "
    "без объяснения. Если, чтобы оно заиграло, нужна подпись, — выбрасывай. "
    "Остаётся только то, что щёлкает само.\n"
    "ВТОРОЙ КРИТЕРИЙ: бери не самое разумное, а самое запоминающееся. "
    "Гладкие и безопасные выброси первыми.\n"
    "КВОТА НА СБОРКУ: минимум ДВА из трёх должны быть СОБРАНЫ — сплав по общему слогу, "
    "слово внутри слова или сдвиг буквы. Просто найденный термин или жаргонизм — "
    "это находка, а не работа; такого бери не больше одного. "
    "Найденное безопаснее, поэтому соблазн взять три находки будет: не поддавайся.\n"
    "Отклоняй всё длиннее двух слов, а также предложения и восклицания.\n"
    "Можешь слегка доточить выбранное.\n"
    "Формат ответа строго такой, без вступления:\n"
    "**Название**\nодна строка — из чего собрано и почему попадает. "
    "Строго не длиннее 15 слов.\n\n"
...
LAB_PICK_IDEA = (
    "\n\nПеред тобой запрос и список черновых идей. Выбери ОДНУ и разверни её. "
    "Остальные не упоминай.\n"
    "ГЛАВНЫЙ КРИТЕРИЙ: самая запоминающаяся, а не самая разумная. "
    "Побеждает та, которую захочется пересказать другому человеку. "
    "Разумную и аккуратную выброси первой.\n"
    "НО НЕ ЗА ЧУЖОЙ СЧЁТ: отклоняй идеи, где берут чужие работы без спроса, "
    "давят на страх или стыд конкретных живых людей, пишут незнакомым в личку "
    "без приглашения. Подшучивать над индустрией целиком или над собой можно, "
    "над конкретным человеком — нет.\n"
    "Выполнимость проверь ПОСЛЕ выбора: если у победившей идеи есть очевидный "
    "стопор, убери именно его, а саму идею не заменяй на безопасную.\n"
    "Формат строго такой, без вступления:\n"
    "**Что это:**\nодно предложение, не длиннее 25 слов.\n\n"
    "**Неожиданный поворот:**\nодно предложение, не длиннее 30 слов.\n\n"
    "**Почему сработает:**\nдве фразы, конкретно и с иронией. Не длиннее 40 слов."
)

# Черновик пишем горячо, отбор — чуть холоднее. На 0.8 отбор оказался слишком
# осторожным: выбирал разумное вместо яркого, поэтому подняли до 0.95.
LAB_PICK_TEMP = float(os.getenv("LAB_PICK_TEMP", "0.95"))
...
def run_lab(message: str, hist: list[dict], category: str) -> str:
    ...
    if category == "название":
        draft_sys = SYSTEM_LAB + LAB_DRAFT_NAMES.format(
            langs=", ".join(random.sample(NAME_LANGS, 4)),
            strata=", ".join(random.sample(NAME_STRATA, 2)))
